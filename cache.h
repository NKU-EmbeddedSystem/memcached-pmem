...
#define cache_create(a, b, c, d, e)                                            \
  umem_cache_create((char *)a, b, c, d, e, NULL, NULL, NULL, 0)
 ...
 * purposes and may help you track down what kind of object you have problems
 * with (buffer overruns, leakage etc)
 * @param bufsize the size of each object in the cache
 * @param align the alignment requirements of the objects in the cache.
 * @param constructor the function to be called to initialize memory when we
 * need to allocate more memory from the os.
 ...
 * allocated with cache_alloc by using cache_free before calling this function.
 * Not doing so results in undefined behavior (the buffers may or may not be
 * invalidated)
 *
