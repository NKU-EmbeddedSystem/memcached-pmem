...
typedef struct {
  void *
      c; /* original connection structure. still with source thread attached. */
  int sfd;       /* client fd. */
...
crawler_module_reg_t crawler_metadump_mod = {.init = NULL,
...
static void *item_crawler_thread(void *arg) {
  ...
  while (do_run_lru_crawler_thread) {
    ...
    if (active_crawler_mod.mod != NULL) {
      ...
      while (active_crawler_mod.c.c != NULL &&
             bipbuf_used(active_crawler_mod.c.buf)) {
...
int lru_crawler_start(uint8_t *ids, uint32_t remaining,
  ...
  STATS_UNLOCK();
  if (is_running && !(type == CRAWLER_AUTOEXPIRE &&
                      active_crawler_type == CRAWLER_AUTOEXPIRE)) {
    pthread_mutex_unlock(&lru_crawler_lock);
...
enum crawler_result_type lru_crawler_crawl(char *slabs,
                                           const enum crawler_run_type type,
  ...
  if (strcmp(slabs, "all") == 0) {
    ...
    for (char *p = strtok_r(slabs, ",", &b); p != NULL;
         p = strtok_r(NULL, ",", &b)) {
  ...
  starts = lru_crawler_start(tocrawl, settings.lru_crawler_tocrawl, type, NULL,
                             c, sfd);
  if (starts == -1) {
