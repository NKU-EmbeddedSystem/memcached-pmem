...
typedef struct _store_page {
  ...
  unsigned int
      written; /* item offsets can be past written if wbuf not flushed */
...
struct store_engine {
  ...
  store_page *
      *page_buckets; /* stack of pages currently allocated to each bucket */
  size_t page_size;
...
static uint64_t _next_version(store_engine *e) { return e->version++; }
...
static store_page *_allocate_page(store_engine *e, unsigned int bucket) {
  assert(!e->page_buckets[bucket] ||
         e->page_buckets[bucket]->allocated == e->page_size);
...
void extstore_close_page(void *ptr, unsigned int page_id,
  ...
  while (1) {
    ...
    while (cur_io) {
      ...
      case OBJ_IO_READ:
        ...
        if (do_op) {
          if (cur_io->iov == NULL) {
            ret = pread(p->fd, cur_io->buf, cur_io->len,
                        p->offset + cur_io->offset);
          } else {
            ret = preadv(p->fd, cur_io->iov, cur_io->iovcnt,
                         p->offset + cur_io->offset);
          }
        }
        break;
      case OBJ_IO_WRITE:
        ...
        ret =
            pwrite(p->fd, cur_io->buf, cur_io->len, p->offset + cur_io->offset);
        break;
      }
      if (ret == 0) {
