...
item *do_item_alloc_pull(const size_t ntotal, const unsigned int id) {
  ...
  for (i = 0; i < 10; i++) {
    ...
    it = slabs_alloc(ntotal, id, &total_bytes,
                     0); // 此处不能只申请dram，也要申请pmem才可以
                         //  否则，lru机制即不发挥作用了
...
struct mem_pair *do_item_alloc_pull_new(const size_t ntotal,
  ...
  for (i = 0; i < 10; i++) {
    ...
    ret = slabs_alloc_new(ntotal, id, &total_bytes,
                          0); // 此处不能只申请dram，也要申请pmem才可以
                              //  否则，lru机制即不发挥作用了
    // gettimeofday(&end, NULL);
    // slab_alloc = slab_alloc + (1000000 * (end.tv_sec - start.tv_sec) +
    ...
    if (it == NULL) {
      ...
      // gettimeofday(&end, NULL);
      // lru_move = lru_move + (1000000 * (end.tv_sec - start.tv_sec) +
      // end.tv_usec - start.tv_usec);
    } else {
      break;
    }
...
item *do_item_alloc(char *key, const size_t nkey,
                    const unsigned int flags, // 核心操作, 控制内存的核心操作
  ...
  if (ntotal > settings.slab_chunk_size_max) {
     ...
     */
    //    printf("will call do_item_alloc_pull in do_item_alloc with
    ...
    if (it != NULL)
      atomic_store(&it->it_flags,
                   atomic_load(&it->it_flags) |
                       ITEM_CHUNKED); // 1 setting ITEM_CHUNKED is here
  ...
  atomic_store(
      &it->it_flags,
      atomic_load(&it->it_flags) |
          (settings.use_cas ? ITEM_CAS : 0)); // setting ITEM_CAS is here
...
struct mem_pair *do_item_alloc_new(char *key, const size_t nkey,
...
int do_item_link(item *it, const uint32_t hv) {
  ...
  ITEM_set_cas(it, (settings.use_cas) ? get_cas_id() : 0);
  // printf("begin assoc_insert, item_linkq, refcount_incr, item_stats_size_add
  // function\n");
  assoc_insert(it, hv);
  item_link_q(it);
  refcount_incr(it);
  item_stats_sizes_add(it);
  // printf("end assoc_insert, item_linkq, refcount_incr, item_stats_size_add
  // function\n");
  ...
  if (settings.lru_segmented) {
    ...
    if ((atomic_load(&it->it_flags) & ITEM_LINKED) != 0) {
      if (ITEM_lruid(it) == COLD_LRU &&
          (atomic_load(&it->it_flags) & ITEM_ACTIVE)) {
        it->time = current_time;
        item_unlink_q(it);
        ...
        if (settings.lru_segmented) {
          if ((atomic_load(&it->it_flags) & ITEM_ACTIVE) == 0) {
            if ((atomic_load(&it->it_flags) & ITEM_FETCHED) == 0) {
              atomic_store(&it->it_flags,
                           atomic_load(&it->it_flags) | ITEM_FETCHED);
            } else {
              atomic_store(&it->it_flags,
                           atomic_load(&it->it_flags) | ITEM_ACTIVE);
              if (ITEM_lruid(it) != COLD_LRU) {
  ...
  for (; tries > 0 && search != NULL; tries--, search = next_it) {
    ...
    case WARM_LRU:
      ...
      if ((atomic_load(&search->it_flags) & ITEM_ACTIVE) != 0) {
        atomic_store(&search->it_flags,
                     atomic_load(&search->it_flags) & ~ITEM_ACTIVE);
        removed++;
        if (cur_lru == WARM_LRU) {
