...
static const entry_details default_entries[] = {
    ...
    [LOGGER_CRAWLER_STATUS] =
        {LOGGER_TEXT_ENTRY, 512, LOG_SYSEVENTS,
         "type=lru_crawler crawler=%d lru=%s low_mark=%llu next_reclaims=%llu "
         "since_run=%u next_run=%d elapsed=%u examined=%llu reclaimed=%llu"},
    [LOGGER_SLAB_MOVE] = {LOGGER_TEXT_ENTRY, 512, LOG_SYSEVENTS,
                          "type=slab_move src=%d dst=%d"},
#ifdef EXTSTORE
    [LOGGER_EXTSTORE_WRITE] = {LOGGER_EXT_WRITE_ENTRY, 512, LOG_EVICTIONS,
                               NULL},
    ...
    [LOGGER_COMPACT_READ_END] = {LOGGER_TEXT_ENTRY, 512, LOG_SYSEVENTS,
                                 "type=compact_read_end id=%lu offset=%llu "
                                 "rescues=%lu lost=%lu skipped=%lu"},
...
static int _logger_thread_parse_ise(logentry *e, char *scratch) {
  ...
  const char *const status_map[] = {"not_stored", "stored",    "exists",
                                    "not_found",  "too_large", "no_memory"};
  const char *const cmd_map[] = {"null",   "add",     "set", "replace",
                                 "append", "prepend", "cas"};
  ...
  total = snprintf(scratch, LOGGER_PARSE_SCRATCH,
                   ...
                   (unsigned long long)e->gid, keybuf, status_map[le->status],
                   cmd, le->ttl, le->clsid);
  return total;
}
...
static int _logger_thread_parse_ige(logentry *e, char *scratch) {
  ...
  char keybuf[KEY_MAX_LENGTH * 3 + 1];
  const char *const was_found_map[] = {"not_found", "found", "flushed",
                                       "expired"};

  uriencode(le->key, keybuf, le->nkey, LOGGER_PARSE_SCRATCH);
  total = snprintf(
      scratch, LOGGER_PARSE_SCRATCH,
      "ts=%d.%d gid=%llu type=item_get key=%s status=%s clsid=%u\n",
      (int)e->tv.tv_sec, (int)e->tv.tv_usec, (unsigned long long)e->gid, keybuf,
      was_found_map[le->was_found], le->clsid);
  return total;
}
...
static int _logger_thread_parse_ee(logentry *e, char *scratch) {
  ...
  uriencode(le->key, keybuf, le->nkey, LOGGER_PARSE_SCRATCH);
  total = snprintf(scratch, LOGGER_PARSE_SCRATCH,
                   "ts=%d.%d gid=%llu type=eviction key=%s fetch=%s ttl=%lld "
                   "la=%d clsid=%u\n",
                   (int)e->tv.tv_sec, (int)e->tv.tv_usec,
                   (unsigned long long)e->gid, keybuf,
                   (le->it_flags & ITEM_FETCHED) ? "yes" : "no",
...
static int _logger_thread_parse_extw(logentry *e, char *scratch) {
  ...
  total = snprintf(
      scratch, LOGGER_PARSE_SCRATCH,
      "ts=%d.%d gid=%llu type=extwrite key=%s fetch=%s ttl=%lld la=%d clsid=%u "
      "bucket=%u\n",
      (int)e->tv.tv_sec, (int)e->tv.tv_usec, (unsigned long long)e->gid, keybuf,
      (le->it_flags & ITEM_FETCHED) ? "yes" : "no", (long long int)le->exptime,
      le->latime, le->clsid, le->bucket);

  return total;
}
#endif
/* Completes rendering of log line. */
static enum logger_parse_entry_ret
logger_thread_parse_entry(logentry *e, struct logger_stats *ls, char *scratch,
                          int *scratch_len) {
  int total = 0;
...
static void logger_thread_write_entry(logentry *e, struct logger_stats *ls,
  ...
  for (x = 0; x < WATCHER_LIMIT; x++) {
     ...
     * flag periodically.
     */
    while (!w->failed_flush && (skip_scr = (char *)bipbuf_request(
                                    w->buf, scratch_len + 128)) == NULL) {
  ...
  for (x = 0; x < WATCHER_LIMIT; x++) {
    ...
    if ((data = bipbuf_peek_all(w->buf, &data_size)) != NULL) {
      if (watchers_pollfds[nfd].revents & (POLLHUP | POLLERR)) {
        ...
        L_DEBUG(
            "LOGGER: poll() wrote %d to %d (data_size: %d) (bipbuf_used: %d)\n",
            total, w->sfd, data_size, bipbuf_used(w->buf));
        if (total == -1) {
...
static int start_logger_thread(void) {
  ...
  if ((ret = pthread_create(&logger_tid, NULL, logger_thread, NULL)) != 0) {
...
static void _logger_log_evictions(logentry *e, item *it) {
  struct logentry_eviction *le = (struct logentry_eviction *)e->data;
  le->exptime = (it->exptime > 0) ? (long long int)(it->exptime - current_time)
                                  : (long long int)-1;
 ...
 */
static void _logger_log_item_get(logentry *e, const int was_found,
                                 const char *key, const int nkey,
                                 const uint8_t clsid) {
  struct logentry_item_get *le = (struct logentry_item_get *)e->data;
...
static void _logger_log_item_store(logentry *e,
                                   const enum store_item_type status,
                                   const int comm, char *key, const int nkey,
                                   rel_time_t ttl, const uint8_t clsid) {
  struct logentry_item_store *le = (struct logentry_item_store *)e->data;
  le->status = status;
  le->cmd = comm;
  le->nkey = nkey;
 ...
 * caller's code.
 */
enum logger_ret_type logger_log(logger *l, const enum log_entry_type event,
                                const void *entry, ...) {
  bipbuf_t *buf = l->buf;
  bool nospace = false;
  va_list ap;
 ...
 */
enum logger_add_watcher_ret logger_add_watcher(void *c, const int sfd,
                                               uint16_t f) {
  int x;
