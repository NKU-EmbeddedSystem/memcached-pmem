...
void print_item_stat(item *it) {
  ...
  if (atomic_load(&it->it_flags) & ITEM_CAS) {
    printf("ITEM_CAS, ");
  }
  if (atomic_load(&it->it_flags) & ITEM_SLABBED) {
    printf("ITEM_SLABBED, ");
  }
  if (atomic_load(&it->it_flags) & ITEM_FETCHED) {
    printf("ITEM_FETCHED, ");
  }
  if (atomic_load(&it->it_flags) & ITEM_ACTIVE) {
    printf("ITEM_ACTIVE, ");
  }
  if (atomic_load(&it->it_flags) & ITEM_CHUNKED) {
    printf("ITEM_CHUNKED, ");
  }
  if (atomic_load(&it->it_flags) & ITEM_CHUNK) {
    printf("ITEM_CHUNK, ");
  }
#ifdef PSLAB
...
static int add_iov(conn *c, const void *buf, int len) {
  ...
  if (IS_UDP(c->transport)) {
    do {
      ...
      if (m->msg_iovlen == IOV_MAX || (c->msgbytes >= UDP_MAX_PAYLOAD_SIZE)) {
        add_msghdr(c);
        m = &c->msglist[c->msgused - 1];
      }
...
static void complete_nread_ascii(conn *c) { // 核心操作, link的核心操作
  ...
  /*
      ...
      for(unsigned long long int c = 0; c < simu_cycles; c++){
              rand_id = rand() % simu_num_chunks; // simu_num_chunks // 保证 256
  byte 对齐 //
  random范围过大，访存性能很差，memcpy次数减少，会看不出access的性能差异 char
  *dst = simu_pslab_pool + rand_id * simu_aligns;
              pmem_memcpy_persist((char*)dst, (char*)it, slabclass_chunksize);
              // pmem_persist(dst, item_size);
      }
      // end
      char *pslab_addr = (char*)it + pslab_dslab_offset; // c->item 应该是从每个
  slabpage 的最后一个 item 开始摘取, 也有可能摘到的是中间的 item // 地址转换
      pmem_memcpy_persist((char*)pslab_addr, (char*)it, slabclass_chunksize);


      c->item = pslab_addr;
      ret = store_item((item*)pslab_addr, comm, c); // 涉及 hash table & lru
  list 的 link 操作 // 修改了 do_store_item 的实现 // lxdchange
  ...
  */
  ...
  // simu_num_chunks = settings.slab_page_size / simu_aligns; // simu_aligns =
  // 256 or 64; simu_aligns = (simu_aligns > 256) ? simu_aligns : 256;
  simu_aligns = write_size;
  simu_num_chunks = (simu_pslab_pool_size - 4096) / simu_aligns;
  // unsigned long long int each_chunks = simu_num_chunks /
  // settings.num_threads; unsigned long long int base_chunks = each_chunks *
  // thread_id; char *dst = (char*)malloc(1024);
  ...
  for (int r = 0; r < repeat; r++) {
    for (unsigned long long int c = 0; c < 4; c++) {
      // int rand_id = rand_r(&seed) % simu_num_chunks; // simu_num_chunks //
      // 保证 256 byte 对齐 //
      // random范围过大，访存性能很差，memcpy次数减少，会看不出access的性能差异
      // printf("thread id is :%llu, rand_id is %d\n", (unsigned long long
      // int)pthread_self(), (int)rand_id); rand_id = (rand_id + 1) %
      // simu_num_chunks;
      unsigned long long int cur_chunk =
          (simu_index[thread_id] * settings.num_threads + thread_id) %
          simu_num_chunks;
      // unsigned long long int cur_chunk = base_chunks + (simu_index[thread_id]
      // % each_chunks); printf("pthread_id: %llu, cur_chunk: %llu\n",(unsigned
      // long long int)pthread_self(), (unsigned long long int)cur_chunk);
  ...
  // mem_access = (1000000 * (end.tv_sec - start.tv_sec) + end.tv_usec -
  // start.tv_usec); printf("mem_access_time: %llu\n", mem_access);

  pmem_memcpy_persist(pmem_ptr, it, size);
  // gettimeofday(&end, NULL);
  // mem_access = mem_access + (1000000 * (end.tv_sec - start.tv_sec) +
...
static void process_bin_get_or_touch(conn *c) {
  ...
  int should_touch =
      (c->cmd == PROTOCOL_BINARY_CMD_TOUCH ||
       c->cmd == PROTOCOL_BINARY_CMD_GAT || c->cmd == PROTOCOL_BINARY_CMD_GATK);
...
static void dispatch_bin_command(conn *c) {
  ...
  case PROTOCOL_BINARY_CMD_GATKQ:
    ...
    break;
  default:
    write_bin_error(c, PROTOCOL_BINARY_RESPONSE_UNKNOWN_COMMAND, NULL, bodylen);
  }
...
static void process_bin_update(conn *c) {
  ...
  if (it == 0) {
    ...
    LOGGER_LOG(c->thread->l, LOG_MUTATIONS, LOGGER_ITEM_STORE, NULL, status, 0,
               key, nkey, req->message.body.expiration, ITEM_clsid(it));
  ...
  if (s_it->it_flags & ITEM_CHUNKED) {
    ...
    while (sch && dch && remain) {
      ...
      if (is_pslab == 0)
        ...
        pmem_memcpy_nodrain(dch->data + dch->used, sch->data + copied,
                            todo); // lxdchange
#endif
      dch->used += todo;
      copied += todo;
    ...
    while (len > done && dch) {
      ...
      if (is_pslab == 0)
        ...
        pmem_memcpy_nodrain(dch->data + dch->used, ITEM_data(s_it) + done,
                            todo); // lxdchange
#endif
      done += todo;
      dch->used += todo;
  ...
  if (comm == NREAD_APPEND) {
    if (new_it->it_flags & ITEM_CHUNKED) {
      ...
      if (new_it->it_flags & ITEM_PSLAB) { // lxdchange
        pmem_memcpy_nodrain(ITEM_data(new_it), ITEM_data(old_it),
                            old_it->nbytes);
        pmem_memcpy_nodrain(ITEM_data(new_it) + old_it->nbytes - 2 /* CRLF */,
...
#ifdef PSLAB
      if (new_it->it_flags & ITEM_PSLAB) { // lxdchange
        pmem_memcpy_nodrain(ITEM_data(new_it), ITEM_data(add_it),
                            add_it->nbytes);
        pmem_memcpy_nodrain(ITEM_data(new_it) + add_it->nbytes - 2 /* CRLF */,
...
enum store_item_type do_store_item(item *it, int comm, conn *c,
  ...
  // gettimeofday(&start, NULL);
  item *old_it =
      do_item_get(key, it->nkey, hv, c,
                  DONT_UPDATE); // 在没有 link 到 hash 表的时候是查找不到的
  // gettimeofday(&end, NULL);
  // hash_find = hash_find + (1000000 * (end.tv_sec - start.tv_sec) +
  // end.tv_usec - start.tv_usec);
  ...
  if (old_it != NULL && comm == NREAD_ADD) {
    ...
    if (stored == NOT_STORED && failed_alloc == 0) {
      ...
      if (old_it != NULL) {
        ...
        do_item_link(it, hv);
        // gettimeofday(&end, NULL);
        // link_time = link_time + (1000000 * (end.tv_sec - start.tv_sec) +
        // end.tv_usec - start.tv_usec);
      }
...
static void server_stats(ADD_STAT add_stats, conn *c) {
  ...
  if (c->thread->storage) {
    STATS_LOCK();
    APPEND_STAT("extstore_compact_lost", "%llu",
                (unsigned long long)stats.extstore_compact_lost);
    APPEND_STAT("extstore_compact_rescues", "%llu",
                (unsigned long long)stats.extstore_compact_rescues);
    APPEND_STAT("extstore_compact_skipped", "%llu",
                (unsigned long long)stats.extstore_compact_skipped);
    STATS_UNLOCK();
    extstore_get_stats(c->thread->storage, &st);
    APPEND_STAT("extstore_page_allocs", "%llu",
                (unsigned long long)st.page_allocs);
    APPEND_STAT("extstore_page_evictions", "%llu",
                (unsigned long long)st.page_evictions);
    APPEND_STAT("extstore_page_reclaims", "%llu",
                (unsigned long long)st.page_reclaims);
    APPEND_STAT("extstore_pages_free", "%llu",
                (unsigned long long)st.pages_free);
    APPEND_STAT("extstore_pages_used", "%llu",
                (unsigned long long)st.pages_used);
    APPEND_STAT("extstore_objects_evicted", "%llu",
                (unsigned long long)st.objects_evicted);
    APPEND_STAT("extstore_objects_read", "%llu",
                (unsigned long long)st.objects_read);
    APPEND_STAT("extstore_objects_written", "%llu",
                (unsigned long long)st.objects_written);
    APPEND_STAT("extstore_objects_used", "%llu",
                (unsigned long long)st.objects_used);
    APPEND_STAT("extstore_bytes_evicted", "%llu",
                (unsigned long long)st.bytes_evicted);
    APPEND_STAT("extstore_bytes_written", "%llu",
                (unsigned long long)st.bytes_written);
    APPEND_STAT("extstore_bytes_read", "%llu",
                (unsigned long long)st.bytes_read);
    APPEND_STAT("extstore_bytes_used", "%llu",
                (unsigned long long)st.bytes_used);
    APPEND_STAT("extstore_bytes_fragmented", "%llu",
                (unsigned long long)st.bytes_fragmented);
    APPEND_STAT("extstore_limit_maxbytes", "%llu",
                (unsigned long long)(st.page_count * st.page_size));
  }
#endif
}
...
static void process_stat_settings(ADD_STAT add_stats, void *c) {
  ...
  APPEND_STAT("inline_ascii_response", "%s",
              settings.inline_ascii_response ? "yes" : "no");
#ifdef EXTSTORE
...
static void conn_to_str(const conn *c, char *buf) {
  ...
  if (!c) {
    ...
    switch (af) {
    case AF_INET:
      (void)inet_ntop(af, &((struct sockaddr_in *)addr)->sin_addr, addr_text,
                      sizeof(addr_text) - 1);
...
static inline item *limited_get(char *key, size_t nkey, conn *c,
                                uint32_t exptime, bool should_touch) {
  item *it;
  if (should_touch) {
...
static inline char *_ascii_get_suffix_buf(conn *c, int i) {
  ...
  if (i >= c->suffixsize) {
    char **new_suffix_list =
        realloc(c->suffixlist, sizeof(char *) * c->suffixsize * 2);
    if (new_suffix_list) {
      c->suffixsize *= 2;
...
static void _get_extstore_cb(void *e, obj_io *io, int ret) {
  ...
  if (ret < 1) {
    ...
    if (io->iov != NULL) {
      ...
      crc2 =
          crc32c(0, (char *)io->iov[0].iov_base + 32, io->iov[0].iov_len - 32);
    ...
    while (key_token->length != 0) {
      ...
      if (it) {
        ...
        if (return_cas || !settings.inline_ascii_response) {
          ...
          nbytes = it->nbytes;
          int suffix_len =
              make_ascii_get_suffix(suffix, it, return_cas, nbytes);
          if (add_iov(c, "VALUE ", 6) != 0 ||
              add_iov(c, ITEM_key(it), it->nkey) != 0 ||
              (settings.inline_ascii_response &&
...
static void process_watch_command(conn *c, token_t *tokens,
                                  const size_t ntokens) {
  uint16_t f = 0;
  int x;
  assert(c != NULL);
...
static void process_memlimit_command(conn *c, token_t *tokens,
                                     const size_t ntokens) {
  uint32_t memlimit;
  assert(c != NULL);
  ...
  if (!safe_strtoul(tokens[1].value, &memlimit)) {
    ...
    if (memlimit < 8) {
      ...
      if (memlimit > 1000000000) {
        out_string(c,
                   "MEMLIMIT_ADJUST_FAILED input value is megabytes not bytes");
        ...
        out_string(c,
                   "MEMLIMIT_ADJUST_FAILED out of bounds or unable to adjust");
      }
    }
  }
}

static void process_lru_command(conn *c, token_t *tokens,
                                const size_t ntokens) {
  uint32_t pct_hot;
  uint32_t pct_warm;
  double hot_factor;
...
#ifdef EXTSTORE
static void process_extstore_command(conn *c, token_t *tokens,
                                     const size_t ntokens) {
  set_noreply_maybe(c, tokens, ntokens);
  bool ok = true;
  if (ntokens < 3) {
...
static void process_command(conn *c, char *command) {
  ...
  if (ntokens >= 3 && ((strcmp(tokens[COMMAND_TOKEN].value, "get") == 0) ||
    ...
    // gettimeofday(&end, NULL);
    // command_parse = command_parse + (1000000 * (end.tv_sec - start.tv_sec) +
    // end.tv_usec - start.tv_usec);
    ...
    process_touch_command(c, tokens, ntokens);
             ...
             (strcmp(tokens[COMMAND_TOKEN].value, "gat") == 0)) {

    process_get_command(c, tokens, ntokens, false, true);

  } else if (ntokens >= 4 &&
             (strcmp(tokens[COMMAND_TOKEN].value, "gats") == 0)) {
    ...
    */
    ...
    if (ntokens == 5 &&
      ...
      out_string(c, "ERROR");
             ...
             strcmp(tokens[COMMAND_TOKEN].value, "lru_crawler") == 0) {
    if (ntokens == 4 && strcmp(tokens[COMMAND_TOKEN + 1].value, "crawl") == 0) {
      ...
      return;
               ...
               strcmp(tokens[COMMAND_TOKEN + 1].value, "tocrawl") == 0) {
      uint32_t tocrawl;
      ...
      return;
               ...
               strcmp(tokens[COMMAND_TOKEN + 1].value, "sleep") == 0) {
      uint32_t tosleep;
    ...
    process_watch_command(c, tokens, ntokens);
             ...
             (strcmp(tokens[COMMAND_TOKEN].value, "cache_memlimit") == 0)) {
    ...
    // commands which exist only for testing the memcached's security protection
             ...
             (strcmp(tokens[COMMAND_TOKEN].value, "misbehave") == 0)) {
    process_misbehave_command(c);
#endif
#ifdef EXTSTORE
  } else if (ntokens >= 3 &&
             strcmp(tokens[COMMAND_TOKEN].value, "extstore") == 0) {
...
static void drive_machine(conn *c) {
  ...
  while (!stop) {
    ...
    case conn_read:
      ...
      // network_io = network_io + (1000000 * (end_here.tv_sec -
      // start_here.tv_sec) + end_here.tv_usec - start_here.tv_usec);
    ...
    case conn_nread:
      ...
      if (!dram_ptr || (((item *)dram_ptr)->it_flags & ITEM_CHUNKED) == 0) {
...
#ifdef PSLAB
        int is_pslab =
            dram_ptr && (((item *)dram_ptr)->it_flags & ITEM_PSLAB) ? 1 : 0;
#endif
        ...
        /*  now try reading from the socket */
        // gettimeofday(&start_here, NULL);
        res = read(c->sfd, c->ritem,
                   c->rlbytes); // item 初始化为 (item) 的 value data 域
        // gettimeofday(&end_here, NULL);
        // network_io = network_io + (1000000 * (end_here.tv_sec -
        // start_here.tv_sec) + end_here.tv_usec - start_here.tv_usec);
  ...
  for (next = ai; next; next = next->ai_next) {
    ...
    if (IS_UDP(transport)) {
      ...
      for (c = 0; c < settings.num_threads_per_udp; c++) {
        ...
        //    printf("will call dispatch_conn_new in server_socket, is udp
        //    transport\n");
...
static int server_sockets(int port, enum network_transport transport,
  ...
  if (settings.inter == NULL) {
    //    printf("will call server_socket in server_sockets with
    //    setting.inter==NULL\n");
    ...
    for (char *p = strtok_r(list, ";,", &b); p != NULL;
      ...
      //    printf("will call server_socket in server_sockets within a
      //    cycle\n");
...
static void usage(void) {
  ...
  printf(
      ...
      "-U, --udp-port=<num>      UDP port to listen on (default: 11211, 0 is "
      "off)\n"
      "-s, --unix-socket=<file>  UNIX socket to listen on (disables network "
      "support)\n"
      "-A, --enable-shutdown     enable ascii \"shutdown\" command\n"
      "-a, --unix-mask=<mask>    access mask for UNIX socket, in octal "
      "(default: 0700)\n"
  ...
  printf("-b, --listen-backlog=<num> set the backlog queue limit (default: "
         "1024)\n");
  ...
  printf(
      ...
      "   - maxconns_fast:       immediately close new connections after "
      "limit\n"
      ...
      "   - slab_chunk_max:      (EXPERIMENTAL) maximum slab size. use extreme "
      "care.\n"
      ...
      "                           small perf hit in ASCII, no perf difference "
      "in\n"
...
int main(int argc, char **argv) {
      ...
      while (*subopts != '\0') {
        ...
        case SLAB_AUTOMOVE_FREERATIO:
          ...
          if (!safe_strtod(subopts_value, &settings.slab_automove_freeratio)) {
            fprintf(stderr,
                    "could not parse argument to slab_automove_freeratio\n");
            return 1;
          }
          break;
        ...
        case PSLAB_POLICY: // 默认是 PSLAB_POLICY_DRAM
          if (strncmp(subopts_value, "dram", 4) ==
              0) { /* strncmp(str1, str2, length), comparison of the first n
                      bytes of two string */
            settings.pslab_policy = PSLAB_POLICY_DRAM;
  ...
  if (storage_file) {
    if (settings.item_size_max > ext_cf.wbuf_size) {
      fprintf(
          stderr,
          "-I (item_size_max: %d) cannot be larger than ext_wbuf_size: %d\n",
    ...
    if (settings.inline_ascii_response) {
      fprintf(stderr,
              "Cannot use inline_ascii_response with extstore enabled\n");
      exit(EX_USAGE);
    }

    if (settings.udpport) {
      fprintf(stderr,
              "Cannot use UDP with extstore enabled (-U 0 to disable)\n");
      exit(EX_USAGE);
    }
  }
#endif
  ...
  */
  ...
  if (pslab_file) {
    if (settings.pslab_recover == true) {
      if (pslab_pre_recover(pslab_file, slab_sizes, MAX_NUMBER_OF_SLAB_CLASSES,
  ...
  int dump_slab_num =
      slabs_dump_sizes(dump_slab_sizes, MAX_NUMBER_OF_SLAB_CLASSES);
  if (dump_slab_num == -1) {
  ...
  mem_slab_pool =
      (struct mem_slab **)malloc(dump_slab_num * sizeof(struct mem_slab *));
  pmem_slab_pool =
      (struct pmem_slab **)malloc(dump_slab_num * sizeof(struct pmem_slab *));
  for (int a = 0; a < dump_slab_num; a++) {
  ...
  thread_index =
      (unsigned long long int *)malloc(16 * sizeof(unsigned long long int));
  // local = 0;
  ...
  if (pslab_file) {
    ...
    if (settings.pslab_recover == false) {
      if (pslab_create(pslab_file, settings.pslab_size, settings.slab_page_size,
                       dump_sizes, dump_num) != 0) {
    ...
    //    printf("Will call slabs_prefill_global_from_pmem after pslab_create in
    //    memcached.c\n");
    slabs_prefill_global_from_pmem();
    //    printf("Will call slabs_update_policy after pslab_create and
    //    prefill_global in memcached.c\n");
    slabs_update_policy();
  }
#endif
#ifdef EXTSTORE
