...
static void process_update_command(conn *c, token_t *tokens,
  ...
  */
  ...
  // prototype: item *item_alloc(char *key, size_t nkey, int flags, rel_time_t
  // exptime, int nbytes);
  ...
  if (it == 0) {
    ...
    LOGGER_LOG(c->thread->l, LOG_MUTATIONS, LOGGER_ITEM_STORE, NULL, status,
               comm, key, nkey, 0, 0);
    /* swallow the data line */
    c->write_and_go = conn_swallow;
    c->sbytes = vlen;
  ...
  ////printf("setting conn->ritem in process_update_command, equals to
  /// ITEM_data(it)\n");
  c->ritem = ITEM_data(it);
  ////printf("setting conn->rlbytes in process_update_command, equals to
  /// it->nbytes, means the length of value\n");
  c->rlbytes = it->nbytes;
  c->cmd = comm;
  conn_set_state(c, conn_nread);
...
#ifdef PSLAB
        int is_pslab =
            (c->item) && (((item *)c->item)->it_flags & ITEM_PSLAB) ? 1 : 0;
#endif
