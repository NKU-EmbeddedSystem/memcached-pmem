...
#ifdef PSLAB
#define SLAB_GLOBAL_PAGE_POOL_PMEM                                             \
  1 /* magic slab class for storing pmem pages for reassignment */
#endif
...
#define ITEM_get_cas(i)                                                        \
  (((i)->it_flags & ITEM_CAS) ? atomic_load(&(i)->data->cas) : (uint64_t)0)
...
#ifdef PSLAB
#define ITEM_dtotal(item)                                                      \
  (item)->nkey + 1 + (item)->nsuffix + (item)->nbytes +                        \
      ((atomic_load(&(item)->it_flags) & ITEM_CAS) ? sizeof(uint64_t) : 0)
#endif
