...
uint64_t pslab_addr2off(void *addr) {
  return ((char *)addr >= (char *)pslab_start)
             ? (char *)addr - (char *)pslab_start
             : 0;
}

#define ADDR_ALIGNED(addr) (((addr) + 256 - 1) & (~255))

#define pslab_off2addr(off)                                                    \
  ((off) ? (void *)((char *)pslab_start + (off)) : NULL)

#define pslab_addr2slab(addr)                                                  \
  ((char *)(addr) >= (char *)pslab_start                                       \
...
void *pslab_get_free_slab(void *slab) {
  ...
  pslab_t *fp =
      PSLAB_SLAB2FRAME(slab); /* dram slab 到 pmem pslab 的转换, 即包装一下 */
  ...
  PSLAB_WALK_FROM(fp, PSLAB_NEXT_FRAME(pslab_pool, cur)) {
    if (atomic_load(&fp->id) == 0 ||
        (fp->flags & (PSLAB_LINKED | PSLAB_CHUNK)) == 0) {
      cur = fp;
...
static void pslab_checksum_init() {
  ...
  pslab_chksum0 = 0;
  pslab_chksum0 +=
      pslab_do_checksum(pslab_pool, offsetof(pslab_pool_t, checksum));
  pslab_chksum0 += pslab_do_checksum_member(pslab_pool, process_started);
  pslab_chksum0 +=
      pslab_do_checksum(&pslab_pool->slab_num,
...
int pslab_do_recover() {
  ...
  settings.oldest_live =
      pslab_pool->flush_time[atomic_load(&pslab_pool->valid) - 1];
  ...
  PSLAB_WALK(fp) { fp->flags = 0; }

  /* check for linked and chunked slabs and mark all chunks */
  PSLAB_WALK(fp) { // 以 Frame 为单位进行遍历,  Frame 是对 original memcached 中
                   // slab_page 的进一步封装, Frame = pslab_t + slab_page
    if (atomic_load(&fp->id) == 0)
      continue;
    size = fp->size;
    ...
    for (i = 0, ptr = fp->slab; i < perslab; i++, ptr += size) {
      ...
      if (atomic_load(&it->it_flags) & ITEM_LINKED) {
        ...
        if (item_is_flushed(it) ||
          ...
          atomic_store(&it->it_flags,
                       ITEM_PSLAB); // 准备后续重新挂载到 PSLAB chain 中
  ...
  PSLAB_WALK(fp) {
    ...
    for (i = 0, ptr = fp->slab; i < perslab; i++, ptr += size) {
      ...
      if ((atomic_load(&it->it_flags) & ITEM_LINKED) &&
          (atomic_load(&it->it_flags) & ITEM_CHUNKED)) {
        // printf("here3\n");
        item_chunk *nch;
  ...
  PSLAB_WALK(fp) {
    ...
    for (i = 0, ptr = fp->slab; i < perslab; i++, ptr += size) {
      ...
      if (atomic_load(&it->it_flags) & ITEM_LINKED) {
        ...
        do_item_relink(it, hash(ITEM_key(it),
                                it->nkey)); // PMEM 中非空闲的item, 直接重新挂载
        ...
        do_slabs_free(
            ptr, 0,
            id); // PMEM 中原来的空闲部分, 还是转换成 dram 挂载到 slabclass id中
      }
    }
  }
...
int pslab_pre_recover(char *name, uint32_t *slab_sizes, int slab_max,
  ...
  if ((pslab_pool = pmem_map_file(name, 0, PMEM_FILE_EXCL, 0, &mapped_len,
                                  &is_pmem)) == NULL) {
  ...
  if (strncmp(pslab_pool->signature, PSLAB_POOL_SIG, PSLAB_POOL_SIG_SIZE) !=
      0) {
  ...
  pslab_end = (pslab_t *)((char *)pslab_start +
                          pslab_pool->slab_num * PSLAB_FRAME_SIZE(pslab_pool));
...
int pslab_create(char *pool_name, uint64_t pool_size,
  ...
  if ((simu_pslab_pool = (char *)pmem_map_file(
           simu_pslab_pool_file_path, simu_pslab_pool_size, PMEM_FILE_CREATE,
           0666, &simu_mapped_len, &simu_is_pmem)) == NULL) {
  ...
  simu_dslab_pool =
      (char *)ADDR_ALIGNED((unsigned long long int)simu_dslab_pool);

  simu_index =
      (unsigned long long int *)(malloc(sizeof(unsigned long long int) * 16));
  for (int i = 0; i < 16; i++) {
    simu_index[i] = 0;
  }
  ...
  ////printf("the pool name is %s, the pool_size is %lu, the slab_page_size is
  ///%u, the slabclass_num is %d\n", pool_name, pool_size, slab_page_size,
  /// slabclass_num);
  size_t mapped_len;
  int is_pmem;
  uint32_t length;
  ...
  if ((pslab_pool = pmem_map_file(pool_name, pool_size, PMEM_FILE_CREATE, 0666,
  ...
  *tmp_pslab_pool = addition;
  ...
  pslab_pool->slab_num =
      (pool_size - pslab_pool->length) / PSLAB_FRAME_SIZE(pslab_pool);
  ...
  for (i = 0; i < slabclass_num; i++) {
    pslab_pool->slabclass_sizes[i] =
        slabclass_sizes[i]; // slabclass_sizes 来源于 dump, dump 来源于 DRAM
                            // slab settings
  }
