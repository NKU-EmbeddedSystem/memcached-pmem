...
#define PSLAB_WALK_FROM(fp, s)                                                 \
  assert(pslab_start != NULL ||                                                \
         ((char *)(s) - (char *)pslab_start) % PSLAB_FRAME_SIZE(pslab_pool) == \
             0);                                                               \
  (fp) = (s) ? (s) : pslab_start;                                              \
...
#define pmem_member_persist(p, m) pmem_persist(&(p)->m, sizeof((p)->m))
#define pmem_member_flush(p, m) pmem_flush(&(p)->m, sizeof((p)->m))
...
int pslab_create(char *pool_name, uint64_t pool_size, uint32_t slab_size,
                 uint32_t *slabclass_sizes, int slabclass_num);
