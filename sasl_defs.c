 ...
 */
const char *const locations[] = {"/etc/sasl/memcached.conf",
                                 "/etc/sasl2/memcached.conf", NULL};
#endif
...
static int sasl_server_userdb_checkpass(sasl_conn_t *conn, void *context,
...
static sasl_callback_t sasl_callbacks[] = {
    ...
    {SASL_CB_SERVER_USERDB_CHECKPASS,
     (sasl_callback_ft)sasl_server_userdb_checkpass, NULL},
#endif
...
void init_sasl(void) {
  ...
  if (memcached_sasl_pwdb == NULL) {
    if (settings.verbose) {
      fprintf(stderr, "INFO: MEMCACHED_SASL_PWDB not specified. "
