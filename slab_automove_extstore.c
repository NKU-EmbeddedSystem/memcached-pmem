...
void *slab_automove_extstore_init(struct settings *settings) {
  ...
  if (a == NULL)
    return NULL;
  a->window_data = calloc(window_size * MAX_NUMBER_OF_SLAB_CLASSES,
...
static void window_global_sum(struct window_global *wg, struct window_global *w,
                              uint32_t size) {
  for (int x = 0; x < size; x++) {
...
static void memcheck(slab_automove *a) {
  ...
  for (int n = 1; n < MAX_NUMBER_OF_SLAB_CLASSES; n++) {
    ...
    unsigned int hold_free =
        (sam->total_pages * sam->chunks_per_page) * a->free_ratio;
...
void slab_automove_extstore_run(void *arg, int *src, int *dst) {
  ...
  *dst = -1;
  ...
  for (n = POWER_SMALLEST; n < MAX_NUMBER_OF_SLAB_CLASSES; n++) {
    bool small_slab = a->sam_before[n].chunk_size < a->item_size ? true : false;
    int w_offset = n * a->window_size;
    struct window_data *wd =
        &a->window_data[w_offset + (a->window_cur % a->window_size)];
    ...
    if (a->sam_after[n].free_chunks >
            a->sam_after[n].chunks_per_page * MIN_PAGES_FOR_RECLAIM &&
        w_sum.dirty == 0) {
      if (small_slab) {
        *src = n;
        *dst = 0;
  ...
  if (wg_sum.pool_high >= a->window_size && !wg_sum.pool_low &&
      youngest != -1) {
    /**src = 0;
     *dst = youngest;*/
    ...
    *src = oldest;
    *dst = 0;
  } else if (!too_free && youngest != -1 && oldest != -1 &&
             youngest != oldest) {
