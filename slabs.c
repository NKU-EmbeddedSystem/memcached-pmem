...
#ifdef EXTSTORE
void slabs_set_storage(void *arg) { storage = arg; }
#endif
/*
...
void slabs_init(const size_t limit, const double factor, const bool prealloc,
  ...
  ////printf("slabs_init function call, memory limit is %d, increase factor is
...
#ifdef PSLAB
int get_slabclass_perslab(int id) { return slabclass[id].perslab; }
...
void slabs_prefill_global_from_pmem(void) {
  ...
  while ((ptr = pslab_get_free_slab(ptr)) !=
         NULL) { // 获取到的 slab 位于 pmem 的 pslab->slab 中, 即可按照 dram
                 // slab 进行管理
    count++;
...
static void
split_slab_page_into_freelist(char *ptr, const unsigned int id) { // lxdchange 1
  // printf("split_slab_page_into_freelist function call, call many times
  // (slabclass_t->perslab) of do_slabs_free function\n");
...
void *get_pmem_page(unsigned int id) {
  ...
  if (pmem_slab_pool[cls_id]->cur_addr == NULL ||
      (pmem_slab_pool[cls_id]->used_slots + cached_size >
       pmem_slab_pool[cls_id]->total_slots)) {
    ...
    // int)pmem_slab_pool[cls_id]->cur_addr);
  }

  char *ret = pmem_slab_pool[cls_id]->cur_addr;
  pmem_slab_pool[cls_id]->cur_addr =
      pmem_slab_pool[cls_id]->cur_addr + cached_size * slabclass[id].size;
  pmem_slab_pool[cls_id]->used_slots =
      pmem_slab_pool[cls_id]->used_slots + cached_size;
...
static int do_slabs_newslab(const unsigned int id) {
  ...
  int len = (settings.slab_reassign ||
  ...
  if ((mem_limit == 0) ||
      (mem_malloced + len > mem_limit && p->slabs > 0 && g->slabs == 0)) {
...
static int do_slabs_newslab_from_pmem(const unsigned int id) { // 核心操作
  ...
  if ((grow_slab_list(id) == 0) ||
      ((ptr = get_page_from_global_pool_by_id(
            SLAB_GLOBAL_PAGE_POOL_PMEM)) // 只是返回之前预分配的一个地址
       == NULL)) {
    MEMCACHED_SLABS_SLABCLASS_ALLOCATE_FAILED(id);
  ...
  // long int)ptr), ((unsigned long long int)ptr)%256); if(pslab_contains(ptr)){
  // printf("ptr is in pmem\n"); }
  ////printf("pmem_memset_persist settings.slab_page_size\n");
  pmem_memset_persist(
      ptr, 0, settings.slab_page_size); // 持久化一整个 slab_page // //
                                        // 得到的仍然是 PMEM 的指针 // 核心操作
  ////printf("end pmem_memset_persist settings.slab_page_size\n");

  // PMEM 到 DRAM 的转换
  split_slab_page_into_freelist(ptr, id); // 由此开始挂载到 slabclass 中
                                          // #ifdef PSLAB
  //     char *dram_ptr = ptr - pslab_dslab_offset; // lxdchange //
  //     由此开始挂载到 slabclass 中 p->slab_list[p->slabs++] = dram_ptr;
...
static int (*newslabs_funcs[])(const unsigned int) = {
    do_slabs_newslab_from_dram, do_slabs_newslab_from_pmem};
...
static int
do_slabs_newslab(const unsigned int id) { // 会对持久内存做改动, 在 spilt
                                          // 的时候, 调用 do_slabs_free() 函数
  ////printf("do_slabs_newslab function call\n");
  int src = slabs_next_src;
  int ret;

  // if(slabs_next_src == PSLAB_POLICY_DRAM){ printf("slabs_next_src==0, choose
  // dram\n"); } else if(slabs_next_src == PSLAB_POLICY_PMEM){
  // printf("slabs_next_src==1, choose pmem\n"); }
...
static void *do_slabs_alloc(const size_t size, unsigned int id,
  ...
  if (p->sl_curr != 0) {
    ...
    // long int)it); printf("After call do_slabs_newslab, Item Stat in
    // do_slabs_alloc\n"); print_item_stat(it);
...
void do_slab_realloc(item *it, unsigned int id) {
  ...
  if (it->it_flags & ITEM_CHUNKED) {
    ...
    ch = ch->next;
    p->requested +=
        it->nkey + 1 + it->nsuffix + sizeof(item) + sizeof(item_chunk);
    while (ch) {
...
#endif
    void
    do_slabs_free(void *ptr, const size_t size,
                  unsigned int id) { // lxdchange 2
...
#ifdef PSLAB
    if (pslab_contains(
            (char *)it)) { // 核心操作, 判断申请到的内存空间是否处于 PMEM
...
struct mem_pair *slabs_alloc_new(const size_t size, unsigned int id,
                                 uint64_t *total_bytes, unsigned int flags) {
 ...
 */
void slabs_mlock(void) { pthread_mutex_lock(&slabs_lock); }

void slabs_munlock(void) { pthread_mutex_unlock(&slabs_lock); }
 ...
 * yet. The memory barrier from the slabs lock should order the key write and
 * the flags to the item? If ITEM_LINKED did exist and was just removed, but we
 * still see it, that's still safe since it will have a valid key, which we then
 * lock, and then recheck everything. This may not be safe on all platforms; If
 * not, slabs_alloc() will need to seed the item key while holding slabs_lock.
 */
  ...
  for (x = 0; x < slab_bulk_check; x++) {
    ...
    case MOVE_FROM_LRU:
      ...
      if ((it->exptime != 0 && it->exptime < current_time) ||
        ...
        /* Expired, don't save. */
        save_item = 0;
      } else if (ch == NULL && (new_it = slab_rebalance_alloc(
                                    ntotal, slab_rebal.s_clsid)) == NULL) {
        /* Not a chunk of an item, and nomem. */
        save_item = 0;
        slab_rebal.evictions_nomem++;
      } else if (ch != NULL && (new_it = slab_rebalance_alloc(
                                    s_cls->size, slab_rebal.s_clsid)) == NULL) {
...
void slabs_rebalancer_pause(void) { pthread_mutex_lock(&slabs_rebalance_lock); }
