...
int lru_maintainer_store(void *storage, const int clsid) {
  ...
  if (slabs_clsid(settings.ext_item_size) > clsid)
    return 0;
  chunks_free = slabs_available_chunks(clsid, &mem_limit_reached, NULL, NULL);
  ...
  if ((it->it_flags & ITEM_HDR) == 0 &&
     ...
     */
    if (hdr_it != NULL) {
      int bucket = (it->it_flags & ITEM_CHUNKED) ? PAGE_BUCKET_CHUNKED
                                                 : PAGE_BUCKET_DEFAULT;
 ...
 */
static int storage_compact_check(void *storage, logger *l, uint32_t *page_id,
...
static void storage_compact_readback(void *storage, logger *l, bool drop_unread,
                                     char *readback_buf, uint32_t page_id,
  ...
  STATS_UNLOCK();
  LOGGER_LOG(l, LOG_SYSEVENTS, LOGGER_COMPACT_READ_END, NULL, page_id, offset,
             rescues, lost, skipped);
}
...
static void *storage_compact_thread(void *arg) {
  ...
  if (l == NULL) {
    fprintf(stderr,
            "Failed to allocate logger for storage compaction thread\n");
    abort();
  }
  ...
  if (readback_buf == NULL) {
    fprintf(
        stderr,
        "Failed to allocate readback buffer for storage compaction thread\n");
    abort();
  }
  ...
  while (1) {
    ...
    if (!compacting &&
        storage_compact_check(storage, l, &page_id, &page_version, &page_size,
                              &drop_unread)) {
      page_offset = 0;
      compacting = true;
      LOGGER_LOG(l, LOG_SYSEVENTS, LOGGER_COMPACT_START, NULL, page_id,
                 page_version);
    }
    ...
    if (compacting) {
      ...
      if (page_offset < page_size && !wrap.done && !wrap.submitted) {
        ...
        LOGGER_LOG(l, LOG_SYSEVENTS, LOGGER_COMPACT_READ_START, NULL, page_id,
                   page_offset);
        storage_compact_readback(storage, l, drop_unread, readback_buf, page_id,
                                 page_version, settings.ext_wbuf_size);
...
void storage_compact_pause(void) { pthread_mutex_lock(&storage_compact_plock); }
...
int start_storage_compact_thread(void *arg) {
  ...
  if ((ret = pthread_create(&storage_compact_tid, NULL, storage_compact_thread,
                            arg)) != 0) {
