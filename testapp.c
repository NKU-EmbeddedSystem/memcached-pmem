...
static enum test_return cache_create_test(void) {
  cache_t *cache =
      cache_create("test", sizeof(uint32_t), sizeof(char *), NULL, NULL);
  assert(cache != NULL);
  cache_destroy(cache);
  return TEST_PASS;
}
...
static enum test_return cache_destructor_test(void) {
  cache_t *cache = cache_create("test", sizeof(uint32_t), sizeof(char *), NULL,
                                cache_destructor);
  assert(cache != NULL);
...
static enum test_return cache_bulkalloc(size_t datasize) {
  cache_t *cache = cache_create("test", datasize, sizeof(char *), NULL, NULL);
...
static pid_t start_server(in_port_t *port_out, bool daemon, int timeout) {
  ...
  */
  char coreadm[128];
  snprintf(coreadm, sizeof(coreadm), "coreadm -p core.%%f.%%p %lu",
...
static enum test_return test_vperror(void) {
  ...
  char expected[80] = {0};
  snprintf(expected, sizeof(expected), "Old McDonald had a farm.  EI EIO: %s\n",
           strerror(EIO));
...
static enum test_return test_issue_102(void) {
  ...
  size_t offset = 5;
  while (offset < 4000) {
    offset += snprintf(buffer + offset, sizeof(buffer) - offset, "%010u ",
                       (unsigned int)offset);
  }
...
static bool safe_recv_packet(void *buf, size_t size) {
  ...
  response->message.header.response.keylen =
      ntohs(response->message.header.response.keylen);
  ...
  response->message.header.response.bodylen =
      ntohl(response->message.header.response.bodylen);
...
static off_t storage_command(char *buf, size_t bufsz, uint8_t cmd,
                             const void *key, size_t keylen, const void *dta,
                             size_t dtalen, uint32_t flags, uint32_t exp) {
...
static off_t ext_command(char *buf, size_t bufsz, uint8_t cmd, const void *ext,
                         size_t extlen, const void *key, size_t keylen,
...
static off_t raw_command(char *buf, size_t bufsz, uint8_t cmd, const void *key,
                         size_t keylen, const void *dta, size_t dtalen) {
  /* all of the storage commands use the same command layout */
  return ext_command(buf, bufsz, cmd, NULL, 0, key, keylen, dta, dtalen);
}

static off_t flush_command(char *buf, size_t bufsz, uint8_t cmd,
                           uint32_t exptime, bool use_extra) {
...
static off_t touch_command(char *buf, size_t bufsz, uint8_t cmd,
                           const void *key, size_t keylen, uint32_t exptime) {
...
static off_t arithmetic_command(char *buf, size_t bufsz, uint8_t cmd,
                                const void *key, size_t keylen, uint64_t delta,
                                uint64_t initial, uint32_t exp) {
...
static void
validate_response_header(protocol_binary_response_no_extras *response,
  ...
  assert(response->message.header.response.datatype ==
  ...
  if (status == PROTOCOL_BINARY_RESPONSE_SUCCESS) {
    ...
    assert(response->message.header.response.cas == 0);
    assert(response->message.header.response.extlen == 0);
    if (cmd != PROTOCOL_BINARY_CMD_GETK && cmd != PROTOCOL_BINARY_CMD_GATK) {
      assert(response->message.header.response.keylen == 0);
    }
  }
}
...
static enum test_return test_binary_noop(void) {
  ...
  size_t len = raw_command(buffer.bytes, sizeof(buffer.bytes),
                           PROTOCOL_BINARY_CMD_NOOP, NULL, 0, NULL, 0);
...
static enum test_return test_binary_quit_impl(uint8_t cmd) {
  ...
  size_t len =
      raw_command(buffer.bytes, sizeof(buffer.bytes), cmd, NULL, 0, NULL, 0);
...
static enum test_return test_binary_set_impl(const char *key, uint8_t cmd) {
  ...
  size_t len = storage_command(send.bytes, sizeof(send.bytes), cmd, key,
                               strlen(key), &value, sizeof(value), 0, 0);
  ...
  send.request.message.header.request.cas =
      receive.response.message.header.response.cas;
  safe_send(send.bytes, len, false);
  if (cmd == PROTOCOL_BINARY_CMD_SET) {
    safe_recv_packet(receive.bytes, sizeof(receive.bytes));
    validate_response_header(&receive.response, cmd,
                             PROTOCOL_BINARY_RESPONSE_SUCCESS);
    assert(receive.response.message.header.response.cas !=
           send.request.message.header.request.cas);
  } else {
    return test_binary_noop();
  }
...
static enum test_return test_binary_replace_impl(const char *key, uint8_t cmd) {
  ...
  len = storage_command(send.bytes, sizeof(send.bytes), PROTOCOL_BINARY_CMD_ADD,
  ...
  len = storage_command(send.bytes, sizeof(send.bytes), cmd, key, strlen(key),
  ...
  for (ii = 0; ii < 10; ++ii) {
    ...
    if (cmd == PROTOCOL_BINARY_CMD_REPLACE) {
      safe_recv_packet(receive.bytes, sizeof(receive.bytes));
      validate_response_header(&receive.response, PROTOCOL_BINARY_CMD_REPLACE,
                               PROTOCOL_BINARY_RESPONSE_SUCCESS);
    }
  }
...
static enum test_return test_binary_delete_impl(const char *key, uint8_t cmd) {
  ...
  size_t len = raw_command(send.bytes, sizeof(send.bytes), cmd, key,
                           strlen(key), NULL, 0);
  ...
  len = raw_command(send.bytes, sizeof(send.bytes), cmd, key, strlen(key), NULL,
                    0);
  safe_send(send.bytes, len, false);
...
static enum test_return test_binary_get_impl(const char *key, uint8_t cmd) {
  ...
  size_t len = ext_command(send.bytes, sizeof(send.bytes), cmd,
                           extlen ? &expiration : NULL, extlen, key,
                           strlen(key), NULL, 0);
...
static enum test_return test_binary_getq_impl(const char *key, uint8_t cmd) {
  ...
  size_t len =
      storage_command(send.bytes, sizeof(send.bytes), PROTOCOL_BINARY_CMD_ADD,
                      key, strlen(key), NULL, 0, 0, 0);
  size_t len2 = ext_command(temp.bytes, sizeof(temp.bytes), cmd,
                            extlen ? &expiration : NULL, extlen, missing,
                            strlen(missing), NULL, 0);
  ...
  len2 = ext_command(temp.bytes, sizeof(temp.bytes), cmd,
                     extlen ? &expiration : NULL, extlen, key, strlen(key),
                     NULL, 0);
  memcpy(send.bytes + len, temp.bytes, len2);
  len += len2;
...
static enum test_return test_binary_incr_impl(const char *key, uint8_t cmd) {
  ...
  size_t len = arithmetic_command(send.bytes, sizeof(send.bytes), cmd, key,
                                  strlen(key), 1, 0, 0);
...
static enum test_return test_binary_version(void) {
  ...
  size_t len = raw_command(buffer.bytes, sizeof(buffer.bytes),
                           PROTOCOL_BINARY_CMD_VERSION, NULL, 0, NULL, 0);
...
static enum test_return test_binary_concat_impl(const char *key, uint8_t cmd) {
  ...
  len = raw_command(send.bytes, sizeof(send.bytes), cmd, key, strlen(key),
                    value, strlen(value));
  safe_send(send.bytes, len, false);
  ...
  assert(receive.response.message.header.response.bodylen ==
         (strlen(key) + 2 * strlen(value) + 4));
...
static enum test_return test_binary_stat(void) {
  ...
  size_t len = raw_command(buffer.bytes, sizeof(buffer.bytes),
                           PROTOCOL_BINARY_CMD_STAT, NULL, 0, NULL, 0);
...
static enum test_return test_binary_pipeline_hickup_chunk(void *buffer,
  ...
  while (hickup_thread_running &&
    ...
    case PROTOCOL_BINARY_CMD_SET:
    case PROTOCOL_BINARY_CMD_SETQ:
      len = storage_command(command.bytes, sizeof(command.bytes), cmd, key,
                            keylen, &value, sizeof(value), 0, 0);
      break;
    case PROTOCOL_BINARY_CMD_APPEND:
    case PROTOCOL_BINARY_CMD_APPENDQ:
    case PROTOCOL_BINARY_CMD_PREPEND:
    case PROTOCOL_BINARY_CMD_PREPENDQ:
      len = raw_command(command.bytes, sizeof(command.bytes), cmd, key, keylen,
                        &value, sizeof(value));
      break;
    case PROTOCOL_BINARY_CMD_FLUSH:
    case PROTOCOL_BINARY_CMD_FLUSHQ:
      len = raw_command(command.bytes, sizeof(command.bytes), cmd, NULL, 0,
                        NULL, 0);
      break;
    case PROTOCOL_BINARY_CMD_NOOP:
      len = raw_command(command.bytes, sizeof(command.bytes), cmd, NULL, 0,
    ...
    case PROTOCOL_BINARY_CMD_INCREMENT:
    case PROTOCOL_BINARY_CMD_INCREMENTQ:
      len = arithmetic_command(command.bytes, sizeof(command.bytes), cmd, key,
                               keylen, 1, 0, 0);
      break;
    case PROTOCOL_BINARY_CMD_VERSION:
    ...
    case PROTOCOL_BINARY_CMD_GATK:
    case PROTOCOL_BINARY_CMD_GATKQ:
      len = touch_command(command.bytes, sizeof(command.bytes), cmd, key,
                          keylen, 10);
      break;
...
static enum test_return test_binary_pipeline_hickup(void) {
  ...
  if ((ret = pthread_create(&tid, NULL, binary_hickup_recv_verification_thread,
                            NULL)) != 0) {
  ...
  size_t len = raw_command(buffer, buffersize, PROTOCOL_BINARY_CMD_QUITQ, NULL,
                           0, NULL, 0);
