...
static void thread_libevent_process(int fd, short which, void *arg) {
  ...
  case 'c':
    ...
    case queue_new_conn: // init_state: conn_new_cmd
      //    printf("will call conn_new in thread_libevent_process with state
      //    queue_new_conn\n");
...
struct mem_pair *item_alloc_new(char *key, size_t nkey, int flags,
                                rel_time_t exptime, int nbytes) {
  struct mem_pair *dram_pmem =
      do_item_alloc_new(key, nkey, flags, exptime, nbytes);
  return dram_pmem;
}
...
enum store_item_type store_item(item *item, int comm, conn *c) {
  ...
  // hash_cal = hash_cal + (1000000 * (end.tv_sec - start.tv_sec) + end.tv_usec
  // - start.tv_usec);
...
void memcached_thread_init(int nthreads, void *arg) {
  ...
  if (power >= hashpower) {
    fprintf(stderr,
            "Hash table power size (%d) cannot be equal to or less than item "
            "lock table (%d)\n",
            hashpower, power);
